...
if (ispc) % Setting up file I/O for windows
    ...
    eegPreprocessDir = [baseDir, 'Code', filesep, 'src', filesep, ...
                      'Preprocessing', filesep, 'Subject', subject, filesep];
...
fname_badchanremoved = [eegPreprocessDir, exptype, '_all_ch_removed2_2048.mat'];
if (exist(fname_badchanremoved, 'file') == 2)
    ...
    for ii=1:n_blocks
        ...
        data=all_filtered(ii);
        figure
        eegplot(data.data,'srate',sr);
    end
    ...
    for ii=1:n_blocks
        ...
        data.setname=['Sub' subject '_' block_names{ii} '_ch_removed2_2048'];
...
fname_reref = [eegPreprocessDir, exptype, '_all_rereferenced_2048.mat'];
if (exist(fname_reref, 'file') == 2)
    ...
    for i=1:n_blocks
        ...
        data.setname=['Sub' subject '_' block_names{i} '_rereferenced_2048'];
...
fname_ICA1 = [eegPreprocessDir, exptype, '_all_ICA1_2048.mat'];
if (exist(fname_ICA1, 'file') == 2)
    ...
    for i=1:n_blocks
        ...
        data=pop_runica(block,'icatype','runica','extended',1,'pca',...
            n_comp(i),'stop',1e-7);
        data.setname=['Sub' subject '_' block_names{i} '_ICA1_2048'];
...
fname_ICA_remove = [eegPreprocessDir, exptype, '_all_IC_visual_removal1_2048.mat'];
if (exist(fname_ICA_remove) == 2)
    ...
    for i=1:n_blocks
        ...
        data.reject=[];
        data.setname=['Sub' subject '_' block_names{i} '_IC_visual_removal1_2048'];
...
resfname = [eegSubjectDir, sprintf('Preprocessed%s_2048.mat', exptype)];
