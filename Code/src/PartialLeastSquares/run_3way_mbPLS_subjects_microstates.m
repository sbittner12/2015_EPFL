...
resfname = [eegDir, 'MicrostateActivations_AllTrials.mat'];
...
if (exist(resfname, 'file') == 2)
    ...
    for i=1:nsubjects
        ...
        Xpls = nprocess(microstate_activations, [1,0], [0,1], [], [], 1, 1);
        Xdata_group{i} = Xpls;
    end
...
for f=1:numFactor
    ...
    [~, ind] = max(Pb_dotprods);
    subplot(rows, 4, [13,14]), bar(Pb{ind}(:,f), 'FaceColor', Pb_colors{ind}), title('Best fit microstate comb');
    xlabel('microstates');
    
    % Plot best fit muscle synergy
    [Qb_dotprods, Qb_colors] = rankQb(Qb, f);
    [~, ind] = max(Qb_dotprods);
    subplot(rows, 4, [15,16]), bar(Qb{ind}(:,f), 'FaceColor', Qb_colors{ind}), title('Best fit synergy');
    ax = gca;
